...
const StatusToggle: React.FC<{ isActive: boolean; onToggle: () => void }> = ({ isActive, onToggle }) => {
  return (
    <button
      onClick={onToggle}
      className={`relative inline-flex items-center h-6 rounded-full w-20 transition-colors duration-300 focus:outline-none ${isActive ? 'bg-poker-green' : 'bg-poker-gray'}`}
...
const Players: React.FC<PlayersProps> = ({ players, onAddPlayer, onUpdatePlayer, onDeletePlayer, onStartGame, onTogglePlayerStatus }) => {
              ...
              sortedPlayers.map(player => (
                <div key={player.id} className={`flex items-center justify-between bg-poker-dark p-4 rounded-lg transition-opacity duration-300 ${!player.isActive ? 'opacity-50' : ''}`}>
                  <div className="flex items-center flex-grow">
                    ...
                    />
                    <label htmlFor={`checkbox-${player.id}`} className="ml-4 flex items-center space-x-3 text-sm flex-wrap flex-grow">
                  ...
                  <div className="flex items-center space-x-3 flex-shrink-0">
                      <StatusToggle isActive={player.isActive} onToggle={() => onTogglePlayerStatus(player.id)} />
