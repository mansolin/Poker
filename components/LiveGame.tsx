...
const LiveGame: React.FC<LiveGameProps> = ({ isLoggedIn, players, allPlayers, gameName, onAddRebuy, onRemoveRebuy, onUpdateFinalChips, onUpdateGameName, onEndGame, onCancelGame, onGoToPlayers, onAddPlayerToGame, onViewProfile }) => {
  ...
  if (players.length === 0) {
    return (
      <div className="text-center p-10 bg-poker-light rounded-lg shadow-xl">
        ...
        {isLoggedIn && <button onClick={onGoToPlayers} className="px-6 py-3 text-white bg-poker-green hover:bg-poker-green/80 font-medium rounded-lg text-sm">Novo Jogo</button>}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-poker-light p-4 rounded-lg shadow-xl gap-4">
          {isEditingName && isLoggedIn ? (
            ...
            <div className="flex items-center space-x-3">
                ...
                {isLoggedIn && <button onClick={() => setIsEditingName(true)} className="text-poker-gray hover:text-poker-gold"><EditIcon /></button>}
            </div>
          )}
          {isLoggedIn && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">{isLoggedIn && <BlindsTimer />}</div>
      </div>
                ...
                {players.map(player => {
                  ...
                  return (
                    <tr key={player.id} className="hover:bg-poker-dark/50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap"><input type="number" min="0" value={player.finalChips} disabled={!isLoggedIn} onChange={(e) => onUpdateFinalChips(player.id, Math.max(0, parseInt(e.target.value, 10) || 0))} onFocus={handleFocus} className="w-24 bg-poker-dark border border-poker-gray/20 text-white text-sm rounded-lg p-2 disabled:bg-poker-dark/50 disabled:cursor-not-allowed" placeholder="0"/></td>
